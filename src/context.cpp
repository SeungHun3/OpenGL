...
void Context::Render()
{
    ...
    m_lightingShadowProgram->SetUniform("lightTransform", lightProjection * lightView); // light 를 기준으로한 lightTransform 넘겨줌
    glActiveTexture(GL_TEXTURE3);
    m_shadowMap->GetShadowMap()->Bind();
    m_lightingShadowProgram->SetUniform("shadowMap", 3); // 3슬롯에 GL_TEXTURE3를 할당
